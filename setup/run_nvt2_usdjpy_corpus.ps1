...
Write-Host 'NVT2 STEP 0: Ground Truth JSON validation'
& $Python (Join-Path $RepoRoot 'tools\nvt\validate_ground_truth.py') '--ground-truth-dir' $gtDir
if ($LASTEXITCODE -ne 0) {
  Write-Host "NVT2 FAIL: Ground Truth validation failed exit=$LASTEXITCODE"
  exit $LASTEXITCODE
}
Write-Host ''

# PowerShell 5.1 does not parse Ground Truth JSON. Python owns UTF-8 JSON parsing.
...
Write-Host 'Ground Truth is syntax-validated before candidate generation.'
