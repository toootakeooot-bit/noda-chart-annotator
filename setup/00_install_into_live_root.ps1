...
Copy-Item "$SourceRoot\tools\pdf_geometry_extractor_compat.py" "$Target\tools\pdf_geometry_extractor_compat.py" -Force
...
Write-Host "PASS: Phase 6-J v0.6.1 PDF geometry patch installed into: $Target"
Write-Host "OpenCV HoughLinesP shape compatibility shim is enabled."
Write-Host "Ollama / OpenAI API / video are NOT used by v0.6.1."
Write-Host "Next: cd `"$Target`""
Write-Host "Then: .\setup\02_verify_pdf_geometry.ps1"
