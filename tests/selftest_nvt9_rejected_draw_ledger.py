...
def main() -> None:
    ...
    assert set(entries) >= {
        ...
        "RD0905-001", "RD0905-002", "RD0905-003",
    }
    ...
    assert entries["RD0912-001"]["status"] == "SUPERSEDED_BY_RETAINED_REFERENCE"
    assert entries["RD0912-001"]["superseded_by"] == "RD0912-011"
    ...
    assert entries["RD0912-007"]["status"] == "SUPERSEDED_SELECTION_DISPLAY_CONFLATION"
    assert entries["RD0912-007"]["superseded_by"] == "RD0912-012"
    assert entries["RD0912-008"]["replacement_policy"] == "VT0912_D1_001_USER_ANNOTATED_2026_LOW_WINDOWS"
    assert entries["RD0912-009"]["status"] == "SUPERSEDED_BY_SCOPED_SYSTEM_CLEANUP"
    assert entries["RD0912-009"]["superseded_by"] == "RD0912-013"
    assert entries["RD0912-010"]["replacement_policy"] == "PAIRWISE_OUTERMOST_WICK_ENVELOPE_ZERO_FORMATION_WICK_BREACH"
    assert entries["RD0912-011"]["replacement_policy"] == "CURRENT_THEN_PREVIOUS_THEN_FROZEN_REFERENCE_REVALIDATED_PRE_CUTOFF"
    ...
    assert entries["RD0905-002"]["replacement_policy"] == "H1_SELECTED_FAMILY_COPIED_TO_M15_NO_RESELECTION"
    assert entries["RD0905-003"]["replacement_policy"] == "ACTIVE_TO_TRANSITION_NO_TL_TO_NEW_ACTIVE_POST_SELECTOR_PRE_MAPPING"
