...
def main() -> int:
    ...
    assert "c = tlColor;" in src
    assert 'StringFind(objectId, "SRC_H4_") >= 0' in src
    assert 'StringFind(objectId, "SRC_H1_") >= 0' in src
    assert 'StringFind(objectId, "SRC_M15_") >= 0' in src
