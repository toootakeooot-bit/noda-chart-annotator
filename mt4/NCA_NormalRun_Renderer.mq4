...
// NCA Normal Run v1 - one-shot MT4 renderer.
// Run AFTER tools/run_normal.py has published a validated snapshot.
// Owns only objects prefixed NCA_DRAW__. User/manual objects are untouched.
...
void OnStart()
{
   ...
   if(tf == "UNSUPPORTED")
   {
      Print("NCA NormalRun Renderer: unsupported chart timeframe. Use D1/H4/H1/M15.");
      return;
   }

   string path = BASE_DIR + "\\NORMAL_" + SafeSymbol(symbol) + "_live_snapshot.csv";

   // Two-pass safety: do not delete the last valid drawing unless a published
   // snapshot exists, has the expected header, and contains renderable rows
   // for this exact XM symbol + chart timeframe.
   int ready = CountRenderableRows(path, symbol, tf);
   if(ready <= 0)
   {
      Print("NCA NormalRun Renderer: no validated renderable rows; keeping existing drawing. code=", ready);
      return;
   }
   ...
   if(rendered < 0)
   {
      Print("NCA NormalRun Renderer: unexpected render failure after validation. code=", rendered);
      return;
   }

   ChartRedraw();
   Print("NCA NORMAL RUN RENDER PASS: ", symbol, " ", tf, " objects=", rendered);
}
