...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build no-lookahead 09/12 current audit package using 600-bar production horizon")
    ...
    write_json(outdir / "selector_trace.json", {
        "schema": "nvt9-0912-current-selector-trace/1.0",
        "audit_id": "ID10IQ200",
        "cutoff_exclusive": cutoff.isoformat(),
        "symbol": args.symbol,
        "timeframes": traces,
    ...
    summary = {
        "schema": "nvt9-0912-current-package/1.0",
        "audit_id": "ID10IQ200",
        "status": "PASS_0912_CURRENT_600BAR_BASE",
        "symbol": args.symbol,
        "cutoff_exclusive": cutoff.isoformat(),
        "window_policy": "LAST_600_CLOSED_BARS_PER_TIMEFRAME",
    ...
    write_json(outdir / "NVT9_0912_CURRENT_SUMMARY.json", summary)
